...
const settings = ['These', 'Do', 'Nothing', 'Duh'];

function ResponsiveAppBar({ collectionNames: pages }) {
  ...
  const navigateTo = (collectionName) => {
	handleCloseNavMenu()
            ...
            >
              {pages.map((page) => (
                <MenuItem key={page} onClick={() => { navigateTo(page) }}>
                  <Typography textAlign="center">{titleCase(page)}</Typography>
                </MenuItem>
              ))}
            </Menu>
          </Box>
